    ...
    QMainWindow(parent)
{
    ...
    insertValueLineEdit->setToolTip("Enter single or multiple values");
...
void MainWindow::deleteClicked() const {
    ...
    if (!this->bst->deleteItem(value.toInt()))
        this->statusLabel->setText("Value is not in the tree...");
    else
        this->statusLabel->setText("Value is deleted.");
...
void MainWindow::insertClicked() const
{
    ...
    while (iterator.hasNext())
    {
        if (!this->bst->insert(iterator.next().toInt()))
            this->statusLabel->setText("Duplicate");
        else
